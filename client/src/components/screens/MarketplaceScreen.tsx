// - Sử dụng mã nguồn từ MarketplaceScreen.tsx
...
export function MarketplaceScreen({ onNavigate }: MarketplaceScreenProps) {
  ...
  const fetchProducts = async () => {
    ...
    try {
      ...
      // Cập nhật: Backend trả về Mảng
      const { data } = await api.get<Book[]>('/products', { params });
      if (Array.isArray(data)) {
        setBooks(data);
        setTotal(data.length); // Tạm thời lấy length vì API mảng không trả về total riêng
      } else {
        setBooks([]);
        setTotal(0);
      }
  ...
  // Giữ nguyên 100% cấu trúc JSX của MarketplaceScreen bên dưới...
  return (
      ...
      <main className="container mx-auto px-4 py-8 flex-1">
        <div className="flex flex-col md:flex-row gap-8">
          ...
          <aside className="w-full md:w-64 space-y-8 h-fit sticky top-24">
            ...
            {/* Các thành phần filter giữ nguyên... */}
            ...
            <div className="space-y-4">
              ...
              <Slider defaultValue={[0, 500000]} max={1000000} step={10000} value={priceRange} onValueChange={setPriceRange} className="py-4" />
              ...
              {(loading ? [] : books).map((book, i) => (
                <div key={book._id} className="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-lg transition-all cursor-pointer group flex flex-col" onClick={() => onNavigate('product-detail', book._id)}>
                  ...
                  <div className="mb-2">
                    ...
                    <h3 className="font-bold text-gray-900 line-clamp-2 text-sm group-hover:text-[#008080] transition-colors">{book.title}</h3>
                  </div>
